...
var
  rft: TRttiFloatType;
  currVal: Currency;
  compVal: Comp absolute currVal; // альтернатива: используем тот же битовый шаблон
begin
  rft := rttiType as TRttiFloatType;
  case rft.FloatType of
    ftSingle:
      Result := TValue.From<Single>(jv.GetValue<Single>);
    ftDouble:
      Result := TValue.From<Double>(jv.GetValue<Double>);
    ftExtended:
      Result := TValue.From<Extended>(jv.GetValue<Extended>);
    ftCurr:
      begin
        currVal := jv.GetValue<Currency>;
        Result := TValue.From<Currency>(currVal);
      end;
    ftComp:
      begin
        currVal := jv.GetValue<Currency>;
        Result := TValue.From<Comp>(compVal); // или PComp(@currVal)^
      end;
  else
    raise Exception.CreateFmt('Unsupported float type %d', [Ord(rft.FloatType)]);
  end;
end;
